...
describe(getGrade.name, () => {
  it('throws on incomplete or invalid parameters', () => {
    // missing keys
    ;[
      {},
      { count: 1 },
  ...
  it('returns an indicator, if the minimum amount is not reached', () => {
    ...
    const actual = getGrade({
      ...
      percent: 0,
      thresholds: [
        { max: 1, name: Random.id() },
        { max: 0, name },
      ],
    })
    expect(actual).to.deep.equal({ name, index: 1, notEnough: true })
  })
  it('throws in case the perc value is not found in thresholds', () => {
    const call = () =>
      getGrade({
        count: 2,
        minCount: 2,
    ...
    expect(call).to.throw(
      'Unexpected code reach: expected 0 to be within defined thresholds.',
    )
  })
  it('returns the respective threshold entry', () => {
    const thresholds = [
      { max: 1.0, name: Random.id() },
      { max: 0.75, name: Random.id() },
      { max: 0.5, name: Random.id() },
      { max: 0.25, name: Random.id() },
      { max: 0.0, name: Random.id() },
    ]
