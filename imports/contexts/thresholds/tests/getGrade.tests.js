...
describe(getGrade.name, function () {
  ...
  it('returns the respective threshold entry', function () {
    ...
    ;[
      { count: 5, minCount: 5, percent: 5 / 5 },
      { count: 5, minCount: 5, percent: 4 / 5 },
      { count: 5, minCount: 5, percent: 3 / 5 },
      { count: 5, minCount: 5, percent: 2 / 5 },
      { count: 5, minCount: 5, percent: 1 / 5 }
    ].forEach((params, index) => {
      params.thresholds = thresholds
      const actual = getGrade(params)
      expect(actual).to.deep.equal({
