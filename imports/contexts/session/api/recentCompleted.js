 ...
 * @return {object[]}
 */
export const recentCompleted = async ({ users, resolve }) => {
  const unique = new Set()
  const query = {
    userId: { $in: users },
    completedAt: { $exists: true },
  }
  const limit = users.length < 15 ? users.length + 1 : 15
  const transform = {
    limit: limit,
    sort: {
      completedAt: -1,
  ...
  const docs = (
    await Session.collection().find(query, transform).fetchAsync()
  ).filter((sessionDoc) => {
  ...
  if (resolve) {
    return mapAsync(docs, async (sessionDoc) => {
      sessionDoc.testCycle = await TestCycle.collection().findOneAsync(
