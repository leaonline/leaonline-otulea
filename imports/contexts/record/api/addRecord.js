...
export const addRecord = async (options) => {
  ...
  const competencyDocsMap = await getCompetencies(
    competencies.map(byCompetencyId),
  )
  const alphaLevelDocsMap = await getAlphaLevels(
    alphaLevels.map(byAlphaLevelId),
  )
