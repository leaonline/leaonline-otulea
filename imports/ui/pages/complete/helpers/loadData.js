...
export const loadData = async ({ sessionId, debug }) => {
  const thresholdRequest = await loadAllContentDocs({ context: Thresholds, debug })
  const thresholdDoc = thresholdRequest[Thresholds.name]
  if (!thresholdDoc) {
    throw new Meteor.Error('error.loadDataFailed', 'loadError.noThresholds')
  }
  const evaluationResults = await callMethod({ name: Session.methods.results, args: { sessionId } })
  if (!evaluationResults) {
    throw new Meteor.Error('error.loadDataFailed', 'loadError.noEvaluationResults')
  }
  ...
  const competencyRequest = await loadAllContentDocs({ context: Competency, ids: competencyIds })
  const competencyDocs = competencyRequest?.[Competency.name] ?? []

  if (competencyDocs.length === 0) {
    throw new Meteor.Error('error.loadDataFailed', 'loadError.competenciesNotFound')
  }
  ...
  const alphaLevelRequest = await loadAllContentDocs({ context: AlphaLevel, ids: alphaLevelIds })
  const alphaLevelDocs = alphaLevelRequest?.[AlphaLevel.name] ?? []

  if (alphaLevelDocs.length === 0) {
    throw new Meteor.Error('error.loadDataFailed', 'loadError.alphaLevelsNotFound')
  }
