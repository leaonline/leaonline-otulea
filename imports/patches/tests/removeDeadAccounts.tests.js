...
describe(removeDeadAccounts.name, () => {
  afterEach(() => {
    restoreAll()
  })
  it('throws if the days is not a valid integer', () => {
    ;[1.1, -1, '1'].forEach((removeOlderThanDays) => {
      expect(() => removeDeadAccounts({ removeOlderThanDays })).to.throw(
        'Match error: Failed Match.Where validation',
      )
    })
  })
  it('removes no users when there are no users older than x days', () => {
  ...
  it('removes users that are older than x days and have no completed session', () => {
    const users = [
      { _id: Random.id() },
      { _id: Random.id() },
      { _id: Random.id() },
    ...
    stub(Session, 'collection', () => ({
      find({ userId, completedAt }) {
        ...
        return userId === users[1]._id ? { count: () => 1 } : { count: () => 0 }
      },
      remove({ userId }) {
    ...
    const { sessionsRemoved, responsesRemoved, usersRemoved } =
      removeDeadAccounts({ removeOlderThanDays: 1 })
  ...
  it('optionally removes dead accounts with started but not completed session', () => {
    ...
    removeDeadAccounts({
      removeOlderThanDays: 1,
      removeIncompleteSessions: true,
    })
    expect(findCalled).to.equal(true)
  })
  it('does not do any db writes in dry-run', () => {
    const users = [
      { _id: Random.id() },
      { _id: Random.id() },
      { _id: Random.id() },
    ...
    const dryRun = true
    const { sessionsRemoved, responsesRemoved, usersRemoved } =
      removeDeadAccounts({ dryRun, removeOlderThanDays: 1 })
