...
describe(addDimensionToFeedback.name, () => {
  before(() => {
  ...
  it('runs through all feedback docs that have no dimension entry yet', () => {
  ...
  it('adds missing testCycles to the result and skips these docs', () => {
    ...
    expect(Feedback.collection().findOne(feedbackId).dimension).to.equal(
      undefined,
    )
  })
  it('updates the dimension from the testCycle to the feedback doc', () => {
    ...
    const feedbackId1 = Feedback.collection().insert({
      testCycle,
      dimension: dimensionId1,
    })
    ...
    expect(Feedback.collection().findOne(feedbackId1).dimension).to.equal(
      dimensionId1,
    )

    // make sure dimensions are assigned
    expect(Feedback.collection().findOne(feedbackId2).dimension).to.equal(
      dimensionId2,
    )
  })
  it('does no db writes if dry-run is active', () => {
    const dimensionId1 = Random.id()
    const dimensionId2 = Random.id()
