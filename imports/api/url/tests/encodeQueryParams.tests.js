...
describe(encodeQueryParams.name, () => {
  it('returns a valid encoded query string for params', () => {
    // no special chars
  ...
  it('returns a valid encoded query string for params with arrays', () => {
    // no special chars
    ...
    // with special chars
    const special1 = encodeQueryParams({
      bar: ['hello+world!=cool', "let's rock (*wow)"],
    })
    expect(special1).to.equal(
      'bar=hello%2Bworld%21%3Dcool&bar=let%27s%20rock%20%28%2Awow%29',
    )
    expect(decodeURIComponent(special1)).to.equal(
      "bar=hello+world!=cool&bar=let's rock (*wow)",
    )
  })
})
