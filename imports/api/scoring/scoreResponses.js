 ...
 */
export const scoreResponses = ({ itemDoc, responseDoc } = {}) => {
  // XXX: compat with subType and subtype
  if (!itemDoc?.value || !(itemDoc?.subtype || itemDoc?.subType)) {
    throw new Meteor.Error(
      'scoreResponses.error',
      'items.expectedItemDefinition',
      responseDoc,
    )
  }

  return Scoring.run(
    itemDoc.subtype || itemDoc.subType,
    itemDoc.value,
    responseDoc,
  )
}
